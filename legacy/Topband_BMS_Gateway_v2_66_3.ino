...
// snprintf into fixed buffer to avoid String churn (addresses M6 pattern).
void sendMqttDiag() {
  if (!g_mqtt_diag || !g_mqtt_enable || !mqtt.connected()) return;

  // Aggregate per-BMS counters (no mutex needed: g_bms_stats is
  // written only by rs485Task on Core 0, we read from Core 1 which
  // may see a slightly stale value - acceptable for diagnostics).
...
<div class="fu"><label class="cr"><input type="checkbox" name="mq_diag" id="i_mq_diag"><span>Publish diagnostics (topic: .../diag, 30s)</span></label></div>
</div>
...
<div class="fu" style="display:flex;justify-content:flex-end;margin-bottom:4px"><button type="button" class="bo" style="padding:4px 10px;font-size:11px" onclick="copyLog()">Copy log</button></div>
<div class="dlog" id="dlog"></div>
<!-- V2.65.1 D2: RS485 Frame Spy -->
<div class="fg" style="margin-top:12px"><div class="fu" style="display:flex;align-items:center;gap:8px;flex-wrap:wrap">
<button type="button" class="bo" id="spyBtn" onclick="spyToggle()">Start RS485 Frame Spy</button>
<button type="button" class="bo" id="spyCopyBtn" style="display:none;padding:4px 10px;font-size:11px" onclick="copySpy()">Copy hex</button>
<button type="button" class="bo" id="spyCloseBtn" style="display:none;padding:4px 10px;font-size:11px" onclick="spyClose()">&times;</button>
<span id="spySt" class="nt" style="font-size:11px;color:var(--tx2);flex:1">Capture RS485 responses from BMS for 5 minutes. Raw hex + decoded current / voltage per frame.</span>
</div></div>
<div id="spyPanel" class="dlog" style="display:none;max-height:280px"></div>
...
var spyTimer=null,spyActive=false;
 ...
 function spySetBtn(active,remainMs){
  ...
  else{b.textContent='Start RS485 Frame Spy';b.style.background='';b.style.color='';
    s.textContent='Capture RS485 responses from BMS for 5 minutes. Raw hex + decoded current / voltage per frame.';s.style.color='var(--tx2)'}
}
...
*/
 ...
 function spyClose(){
  ...
  $('spyCopyBtn').style.display='none';
  $('spyCloseBtn').style.display='none';
}
...
spyRender=function(j){
  ...
  $('spyCopyBtn').style.display=(haveFrames&&panelVisible)?'inline-block':'none';
  $('spyCloseBtn').style.display=(haveFrames&&panelVisible)?'inline-block':'none';
};
...
$('i_mq_en').checked=!!c.mq_en;$('i_mq_full').checked=!!c.mq_full;$('i_mq_diag').checked=!!c.mq_diag;$('i_mq_ip').value=c.mq_ip||'';$('i_mq_pt').value=c.mq_pt;
